...
export const useProvideAuth = () => {
   ...
   const updateUserFriends = (addfriend, friend) => {
      ...
      // console.log('from updateFriends, user ', user.friends);
      const newFriends = user.friends.filter(f => f.to_user !== friend.to_user._id);
...
export const useProvidePosts = () => {
   ...
   const [loading, setLoading] = useState(true);
    ...
    const fetchPosts = async () => {
      const response = await getPosts(1, 8);
   ...
   const addComment = (comment, postId) => {
	...
	return { data: posts, loading, addPostToState, addComment };
}